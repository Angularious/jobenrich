...
- **In-country-first (SOFT, people + recruiters only — NOT alumni):** `inCountryFirst()` stably sorts same-country people to the front so the visible 5 are in-region; out-of-country (and unknown-country) people follow under "+ Show N more". **Removes nobody** — a remote/overseas team still shows everyone, just lower. Bias country = the job's country, **defaulting to US** when location is missing/unparseable, but **no bias for explicitly remote roles** (`sortCountry()`). This complements the existing API-level `location:[country]` filter on the primary steps — the soft sort cleans the looser fallbacks the API filter doesn't cover. (Measured: a company-name search for "Stripe" + "software engineer" returned 21/25 India — the sort surfaces the 2 US engineers into the visible 5.) Alumni are intentionally exempt: you'd reach out to a school alum at the company wherever they now live. Coresignal results lack `country`/current-company in the preview, so they sort to "rest" and skip the ex-employee drop (its `fromCoresignal()` already prefers current-company matches).
